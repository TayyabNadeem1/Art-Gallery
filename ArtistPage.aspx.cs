    ...
    public partial class ArtistPage : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                con.Open();
                cmd.CommandText = "select * from users where name= '" + Session["Username"] + "'";
                cmd.Connection = con;
                sda.SelectCommand = cmd;
                sda.Fill(ds);
                Label1.Text = ds.Tables[0].Rows[0]["Name"].ToString();

                cmd.CommandText = "select picture from users where name= '" + Session["Username"] + "'";
                cmd.Connection = con;
                ...
                cmd.CommandText = "select artpicture from art inner join likedart on art.artid=likedart.ArtID inner join users on likedart.userid=users.userid where users.name='" + Session["Username"] + "'";
                cmd.Connection = con;
